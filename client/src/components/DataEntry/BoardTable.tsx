...
const BoardTable: React.FC<IProps> = ({ boardName, ballots, url }: IProps) => {
  const columns: Column<IBallot>[] = [
        ...
        return ballot.status !== BallotStatus.NOT_AUDITED ? (
          <>
            {ballot.status === BallotStatus.AUDITED ? (
              <p style={grayColor}><Icon icon="tick" /> Audited</p>
            ) : (
              <p style={grayColor}><Icon icon="tick" /> Not Found</p>
            )}
          </>
        ) : (
    ...
    },
    {
      Header: 'Actions',
      accessor: ballot => {
        return ((ballot.status === BallotStatus.AUDITED) || (ballot.status === BallotStatus.NOT_FOUND)) ? (
            <AuditBtn
  ...
  return (
    <div className="board-table-container">
      <HeaderWrapper>
        <HeaderInnerWrapper>
          <LeftSection>
            <p className="bp3-text-large">{totalAudited + totalNotFound} of {ballots.length} ballots have been audited.</p>
            <SuccessLabel className="summary-label">Audited: {totalAudited}</SuccessLabel>
            <WarningLabel className="summary-label">Not Audited: {totalNotAudited}</WarningLabel>
            {totalNotFound > 0 && (
              <DangerLabel className="summary-label">Not Found: {totalNotFound}</DangerLabel>
            )}
          </LeftSection>
          <RightSection>
            {HeaderButton}
      ...
      <Inner>
        <ContentWrapper>
          <TableWrapper>
            ...
            <LinkButton to={`${url}/signoff`} disabled={!roundComplete} intent={roundComplete ? "success" : "none"} style={{float: 'right', marginTop: '10px'}} large>
              Submit Audited Ballots
            </LinkButton>
          </TableWrapper>
          <InstructionsWrapper>
            ...
            <OL>
              ...
              <li>
                Once all ballots are audited, Submit audited ballots. Once results are submitted, no further edits can be made.
              </li>
            </OL>
          </InstructionsWrapper>
        </ContentWrapper>
